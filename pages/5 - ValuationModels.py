...
with st.form(key='Input Assset Info', clear_on_submit=False,
    ...
    if submitted:
        ...
        def CAPM(stock_a, stock_m):
            ...
            st.write('\n')
            st.write(20*'==')
            st.metric('Calculated Beta - Linear Regression: ', beta_reg)
            st.metric('Calculated Alpha: ', alpha)
            st.write(20*'==')
...
try:
    ...
    current_liabilities = bsheet2.loc['Total Liabilities Net Minority Interest'][1]
...
with u1:
    ...
    st.metric("Net Income (Cr.)", net_income)
with u2:
    st.metric("Current Assets (Cr.)", current_assets)
with u3:
    st.metric("Operating Profits (Cr.)", op_profits)
with u4:
    st.metric("Operating Cash Flow", cash)
with u5:
    st.metric("Gross Profit in (Cr.)", gross_profit)
...
with g3:
    st.metric("CR = (Current Assets) ÷ (Current Liabilities)", cr)
...
with g10:
    st.metric("Total Debt", total_debt)
with g11:
...
with h14:
    ...
    st.write('A High ICR i.e. above 2.00 exhibits a firm\'s capacity to payy off all ineterests of short term debts')
with h15:
...
with g17:
    st.metric("**=> ATR = Net Sales/Total Revenues**", atr)
try:
    itr = cogs/inventory
except Exception:
    itr = 'In the Works'
...
with h18:
    st.metric('**=>COGS/Avg. Value of Inventory**', itr)
...
with g19:
    st.metric("=>DSI = (Avg. Value of Inventory/COGS)X365: DSI",dsi)
st.write('-------------------------------------------------------------------')
#################################################
...
with g21:
    st.metric("**=>GMR = (Revenues - COGS)/Revenues**",gmr)
...
with g23:
    st.metric("**=>OPR = Operating Profits*100/Net Revenues",omr)
try:
...
with g25:
    st.metric("ROE = (Net Profit)÷(Avg Shareholder’s Equity)",roe)
...
st.write("**=>DPR = Total Dividends/Net Income**")
