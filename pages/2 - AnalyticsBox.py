...
vidfile2 = open(f"{direc}/pages/appdata/imgs/Analytics-Anime.mp4", "rb")
video_bytes2 = vidfile2.read()
...
def getdata(stock2):
    ...
    df0 = stock2.history(period="2y")
    df0.to_csv(f"{stock2}.csv")
    di = df0.index
    df2 = pd.read_csv(f"{stock2}.csv", header=[0])
    df2 = df2.set_index(['Date'])
    df2 = df2.drop(['Dividends', 'Stock Splits'], axis=1)
    return df0, di, df2


df0, di, df2 = getdata(stock2)
# st.write(stock2)
...
dh14, dh3, df_14L, df_14H, df_3L, df_3H, dl3, dl14, dh_52, dh52, dl_52, dl52, dh_25, dh25, dl_25, dl25 = hi_lo(
    df)
trendlist = pd.Series(["Moving Averages (MA)", "Moving Average Convergence Divergence (MACD)", "Average Directional Movement Index (ADX)", "Vortex Indicator (VI)", "Trix (TRIX)", "Mass Index (MI)",
                      "Commodity Channel Index (CCI)", "Detrended Price Oscillator (DPO)", "KST Oscillator (KST)", "Ichimoku Kinkō Hyō (Ichimoku)", "Parabolic Stop And Reverse (Parabolic SAR)", "Schaff Trend Cycle (STC)"])
...
volatlist = pd.Series(["Average True Range (ATR) & Ulcer Index (UI)",
                      "Bollinger Bands (BB)", "Keltner Channel (KC)", "Donchian Channel (DC)"])
...
with su2:
    ...
    try:
        # 4b
        regularMarketPreviousClose = dfi.at['regularMarketPreviousClose', 'Details']
...
def psar(df):
    ...
    fig_psar_up.update_yaxes(
        title='psar up', visible=True, showticklabels=True)
...
def pvo(df):
    ...
    fig_pvo.add_trace(go.Scatter(
        x=df.index, y=df["momentum_pvo_hist"], name='PVO Histogram', showlegend=False), row=2, col=1)
    fig_pvo.add_trace(go.Scatter(
...
def atr(df):
    ...
    fig_atr_ui.update_xaxes(
        title='Timeline', visible=True, showticklabels=True)
    fig_atr_ui.update_yaxes(title='ATR & Ulcer Index',
                            visible=True, showticklabels=True)
...
def dr(df):
    ...
    fig_dlr.update_layout(height=400, title=f"Direct Logarithmic Returns for {
                          stock}", showlegend=False)
    fig_dr.update_layout(height=400, title=f"Direct and Log Returns for {
                         stock}", showlegend=False)
...
def adi(df):
    ...
    fig_adi.update_layout(height=400, title=f'ADI for {
                          stock}', showlegend=False)
...
def donc(df):
    ...
    fig_dc.update_yaxes(title=f'Donchian Channels for {
                        stock}', showticklabels=True, visible=True)
...
#    df_k_ind = df_k_ind.dropna()
    fig_k = px.line(df_k)
    fig_k.update_yaxes(title='Keltner Bandwidth',
                       showticklabels=True, visible=True)
...
with st.container():
    ...
    st.write("""Measure the rate of change in a stock's price over time.
             Assess the speed and direction of price changes for an asset,
             gauge the asset's inertia.""")
    st.write(
        """Investigate whether an asset has been Overbought or Oversold,
        and to what level.""")
    ...
    if choix2 == "Relative Strength Index (RSI)":
        ...
        with ts3:
            if l_tsi > hi_lim_tsi:
                ...
                st.metric("TSI Signal", "Active Trading Zone!!")
        ...
        with uoc2:
            st.metric("Ultimate Oscillator Index", l_uo.round(2))
        with uoc3:
            st.write("Signal under development")
        ...
        with wrc3:
            if l_wr < -80:
                ...
                st.error("WR Signal", "Overbought")
            else:
                st.warning("Moderately Valued")
        ...
        with ao3:
            # st.write("*AO SIGNAL*")
            if l_ao > 0:
                st.error("In Buy Zone!!")
            if l_ao < 0:
                st.success("In Sell Zone!!")
            else:
        ...
        fig_ppo, fig_ppo_signal = ppo(df)
        st.write("Gauge the momentum of one Price MA curve against another.")
        ...
        fig_pvo, fig_pvo_signal = pvo(df)
        st.write("Gauge the momentum of one Volume MA curve against another.")
...
with st.container():
    ...
    with cvx1:
        ...
        st.write(
            "Compare the spread between any asset's high and low price limits.")
    with cvx2:
...
with st.container():
    ...
    if choix3 == "Average True Range (ATR) & Ulcer Index (UI)":
        ...
        with bx3:
            if l_bbhi == 1:
                st.write("BB Hi Signal")
                st.write("Approaching Highs >> Sell!!")
            else:
                st.write("BB Hi Signal")
                st.write("Status - Stable")
        with bx4:
            if l_bbli == 1:
                st.write("BB Lo Signal")
                st.write("Approaching Lows >> Acquire!!")
            else:
                st.write('BB Lo Signal')
                st.write("No Indications Yet")
        with bx5:
            if l_bbp > 0.7:
                st.write("Percentile Signal")
                st.write("Around the Highs")
            elif l_bbp < 0.3:
                st.write("Percentile Signal")
                st.write("Around the Lows")
            else:
        ...
        with dcc2:
            st.metric("Donchian High", l_dch.round(2))
        with dcc3:
            st.metric("Donchian Med", l_dcm.round(2))
        with dcc4:
            st.metric("Donchian Low", l_dcl.round(2))
        with dx5:
            if l_bbp > 0.7:
...
with st.empty():
    ...
    with cvv1:
        ...
        st.write(
            "Infer market's conviction. Gauge accumulation tendencies, upcoming reversals.")
    with cvv2:
    ...
    if choix5 == "Money Flow Index (MFI)":
        ...
        with vp1:
            ...
            st.write(
                ...
                demand and supply.''')
            st.write(
                '''Percent Price changes measure relativity in supply vs
                demand, Volume indicates the force behind the trend.''')
        with vp2:
...
st.write("  --------------  ")
with st.container():
    st.markdown(''' <div align="center"><h2>3. Company Information,
                Data & Statistics</h1></div>''',
...
with ft9:
 st.write(" ")
with ft10:
 st.write(": | 2023 - 2024 | All Rights Resrved  ©  Ledgr Inc. | www.alphaLedgr.com | alphaLedgr Technologies Ltd. :")
with ft11:
 st.write(" ")
