...
logofile = f"{bpath}/appdata/imgs/Ledgr_Logo.png"
logofile2 = f'{bpath}/appdata/imgs/LOGO.png'
...
with st.form("pfinputs"):
    ...
    exp_returns = st.slider(
        "What levels of Returns are you expecting, realistically",
        0, 100, (15, 35))
    volatility_tolerance_range = st.slider(
        "Indicate the volatility range that you prefer/can afford",
        0, 30, (5, 25))
    ...
    def get_stock(ticker):
        ...
        data = ticker2.history(period='2y')
        data.reset_index()
        data.to_csv(f"{bpath}/appdata/OHLC/{ticker2}_data.csv", index=False)
    ...
    fig_return_global.update_yaxes(title='Optimal Allocations',
                                   visible=True, showticklabels=True)
    ...
    def figs(df, returns, mu, S):
        ...
        figRet.update_layout(title="Historic Returns at Daily Intervals",
                             title_x=0.35)
        figMU = px.bar(df_mu, color=df_mu.index, text_auto='.2M')
        figMU.update_traces(textfont_size=12, textangle=0,
        ...
        figCov.update_layout(title="Risk Profile [Covariance Map]",
                             title_x=0.25)
    ...
    figMVAR = px.pie(values=allocation_mvar,
                     names=names_mvar, hole=0.4)
    ...
    figCLA = px.pie(values=allocation_cla,
                    names=names_cla, hole=0.4)
    figCLA.update_traces(textposition='outside',
