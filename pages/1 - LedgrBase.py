...
with hg2:
    st.video(f'{direc}/pages/appdata/imgs/Marketbase-Anime.mp4')
...
#    units_balance = st.number_input("Input your Units held in Balance", 500)
    if not mf_sel == 'Select Code':
        ...
        sinfo = mf.get_scheme_info(mf_sel)
#        df_snav = mf.get_scheme_historical_nav(f'{mf_sel}',as_Dataframe=True)
#        debt = mf.get_open_ended_debt_scheme_performance(True)
#        equity = mf.get_open_ended_equity_scheme_performance(True)
#        hybrid = mf.get_open_ended_hybrid_scheme_performance(True)
#        solution = mf.get_open_ended_solution_scheme_performance(True)
        amc_details = mf.get_all_amc_profiles(True)


        st.metric('1. Fund_house', fh.iloc[-1])
        st.metric('2. Scheme Type', stype.iloc[-1])
        st.metric('3. Scheme Category', scat.iloc[-1])
        st.metric('4. Scheme Name', sname.iloc[-1])
        st.metric('5. Scheme Code', scode.iloc[-1])
        st.write('6.Quotation', df_q)
        st.write('7. Scheme Info', sinfo)
        st.write("NAV", df_snav)
        f_nav = px.line(df_snav['nav'])
        st.plotly_chart(f_nav)
        with st.expander("AMC Profile Information"):
            st.write(amc_details)
        #          try:
        #            mkt_value_of_units_held = mf.calculate_balance_units_value(mf_sel, units_balance)
        #             with st.expander("Calculate the Returns on your Mutual Funds Here!"):
        #               with st.form():
        #                 balanced_units = st.number_input("Input the number of Balanced Inputs Held", 500)
        #                monthly_sip = st.number_input("Input your average Monthly Deposits or SIPs", 500)
        #               investment_in_months = st.number_input("Input the total number of months the investment has been planned for", 500)
        #              if submitted:
        ##               st.metric("Returns Expected", mf_returns)
          # df_mf.reset_index()
        ...
        st.write("Build a Portfolio Please!!")
st.write("   ----   ")
